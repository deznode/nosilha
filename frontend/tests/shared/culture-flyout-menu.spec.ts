...
test.describe("Culture Flyout Menu - Cultural Heritage Navigation", () => {
  ...
  test("displays all culture menu items when opened with proper positioning", async ({
    page,
  }) => {
    ...
    const flyoutHistoryLink = flyoutPanel.getByRole("link", {
      name: /history of brava/i,
    });
    const flyoutFiguresLink = flyoutPanel.getByRole("link", {
      name: /historical figures/i,
    });
    const flyoutGalleryLink = flyoutPanel.getByRole("link", {
      name: /photo galleries/i,
    });
    ...
    await expect(flyoutHistoryLink).toContainText(
      "Discover the island's rich past"
    );

    await expect(flyoutFiguresLink).toContainText("Historical Figures");
    await expect(flyoutFiguresLink).toContainText(
      "Meet the people who shaped Brava"
    );

    await expect(flyoutGalleryLink).toContainText("Photo Galleries");
    await expect(flyoutGalleryLink).toContainText(
      "Visual stories of Brava's beauty"
    );
  });

  test("culture button shows active state only on culture pages", async ({
    page,
  }) => {
    // Set desktop viewport to see the culture flyout button
    await page.setViewportSize({ width: 1024, height: 768 });
    ...
    const cultureButtonOnHistory = page.getByRole("button", {
      name: /culture/i,
    });
    ...
    const cultureButtonOnPeople = page.getByRole("button", {
      name: /culture/i,
    });
  ...
  test("culture flyout navigation works correctly for tourism discovery", async ({
    page,
  }) => {
    // Set desktop viewport to see the culture flyout button
    await page.setViewportSize({ width: 1024, height: 768 });
  ...
  test("flyout menu works correctly on mobile viewports", async ({ page }) => {
    ...
    if (await mobileMenuButton.isVisible()) {
      ...
      await expect(
        page.getByRole("link", { name: /history of brava/i })
      ).toBeVisible();
      await expect(
        page.getByRole("link", { name: /historical figures/i })
      ).toBeVisible();
      await expect(
        page.getByRole("link", { name: /photo galleries/i })
      ).toBeVisible();
    }
  });
