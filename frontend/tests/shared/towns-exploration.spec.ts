...
test.describe('Towns Exploration - Cultural Heritage Discovery', () => {
  ...
  test('displays overview of Brava Island communities', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /towns.*villages/i })).toBeVisible();
    
    // Check for island communities introduction
    const introSection = page.locator('section').filter({ hasText: /island communities/i });
    ...
    const overviewImage = page.locator('img').filter({ hasText: /panoramic|overview|towns/i }).or(
      page.locator('img[alt*="towns"], img[alt*="settlements"]')
    );
  ...
  test('showcases featured towns with detailed information', async ({ page }) => {
    await page.goto('/towns');
    
    // Look for featured towns section
    const featuredSection = page.locator('section').filter({ hasText: /main communities/i });
    await expect(featuredSection).toBeVisible();
    
    // Should display Nova Sintra and Furna prominently
    const novaSintraCard = page.getByText(/Nova Sintra/i).locator('..').locator('..');
  ...
  test('enables navigation to individual town pages', async ({ page }) => {
    ...
    const townLink = page.getByRole('link', { name: /explore.*nova sintra/i }).or(
      page.getByRole('link', { name: /nova sintra/i })
    );
    ...
    if (await townLink.isVisible()) {
      ...
      await expect(page.getByRole('heading', { name: /nova sintra/i })).toBeVisible();
      ...
      const townCards = page.locator('[data-testid="town-card"]').or(
        page.locator('div').filter({ hasText: /nova sintra|furna/i }).first()
      );
      ...
      if (await townCards.isVisible()) {
        ...
        const hasExpandedContent = await page.locator('[data-testid="town-details"]').isVisible();
  ...
  test('displays hidden gems and smaller communities', async ({ page }) => {
    ...
    const hiddenGemsSection = page.locator('section').filter({ hasText: /hidden gems/i });
    await expect(hiddenGemsSection).toBeVisible();
    
    // Should show smaller communities
    const smallerTowns = ['Mato', 'Cachaço', 'Nossa Senhora do Monte', 'Lima Doce'];
    const pageContent = await page.textContent('body');
    ...
    const hiddenGemCards = hiddenGemsSection.locator('div').filter({ hasText: /residents|population|elevation/i });
  ...
  test('integrates with interactive map for spatial context', async ({ page }) => {
    await page.goto('/towns');
    
    // Look for map integration
    const mapCTA = page.locator('section').filter({ hasText: /interactive map/i });
    ...
    const mapContainer = page.locator('[data-testid="interactive-map"]').or(
  ...
  test('encourages community contribution and engagement', async ({ page }) => {
    ...
    const contributeSection = page.locator('section').filter({ hasText: /share your town stories/i });
    ...
    expect(contributeContent?.toLowerCase()).toMatch(/(photos|experiences|stories|contribute)/);
    
    // Should have link to contribute page
    const contributeLink = page.getByRole('link', { name: /contribute content/i });
  ...
  test('connects to directory entries within towns', async ({ page }) => {
    ...
    const townCards = page.locator('[data-testid="town-card"]').or(
      page.locator('div').filter({ hasText: /nova sintra|furna/i })
    );
    ...
    if (await townCards.count() > 0) {
      ...
      const hasBusinessMention = /restaurant|hotel|business|establishment/i.test(townContent || '');
  ...
  test('preserves cultural authenticity in presentation', async ({ page }) => {
    ...
    expect(pageContent?.toLowerCase()).toMatch(/(heritage|tradition|cultural|maritime|colonial)/);
    
    // Should mention geographic/geological context
    expect(pageContent?.toLowerCase()).toMatch(/(volcanic|crater|mountain|coastline|atlantic)/);
    
    // Check for respectful language about communities
    const hasRespectfulTone = !/(primitive|backward|underdeveloped|poor)/i.test(pageContent || '');
    expect(hasRespectfulTone).toBeTruthy();
    
    // Should emphasize positive community aspects
    expect(pageContent?.toLowerCase()).toMatch(/(resilience|creativity|hospitality|welcome|authentic)/);
  });
  ...
  test('provides practical tourism information', async ({ page }) => {
    ...
    const highlightTags = page.locator('span').filter({ hasText: /UNESCO|Harbor|Pools|Architecture|Views|Heritage/i });
  ...
  test('optimized for mobile cultural exploration', async ({ page, browserName }) => {
    ...
    if (await townCards.count() > 0) {
      ...
      const hasInteraction = await page.locator('[data-testid="town-details"]').isVisible() ||
  ...
  test('handles connectivity issues with graceful degradation', async ({ page }) => {
    await page.goto('/towns');
    ...
    if (await mapLink.isVisible()) {
      ...
      const hasOfflineHandling = await page.locator('text=/offline|unavailable|connection/i').isVisible() ||
  ...
  test('supports SEO and metadata for cultural discoverability', async ({ page }) => {
    await page.goto('/towns');
    ...
    if (metaDescription) {
      expect(metaDescription.toLowerCase()).toMatch(/(brava|cape verde|communities|heritage)/);
    }
    ...
    const hasStructuredData = await page.locator('script[type="application/ld+json"]').isVisible();
    
    if (hasStructuredData) {
      console.log('Structured data present for enhanced search discoverability');
    }
  });
});
...
test.describe('Individual Town Pages', () => {
  ...
  test('displays detailed Nova Sintra information', async ({ page }) => {
    ...
    if (!page.url().includes('404') && !page.url().includes('not-found')) {
      ...
      expect(pageContent?.toLowerCase()).toMatch(/(capital|unesco|colonial|architecture)/);
      ...
      const directoryLinks = page.getByRole('link', { name: /restaurant|hotel|business/i });
      ...
      console.log('Individual town pages not implemented - testing main towns page integration');
    }
  });
});