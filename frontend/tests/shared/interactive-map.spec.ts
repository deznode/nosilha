...
test.describe('Interactive Map - Spatial Tourism Discovery', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="interactive-map"]', { timeout: 15000 });
  ...
  test('loads interactive map centered on Brava Island', async ({ page }) => {
    ...
    const mapCanvas = page.locator('.mapboxgl-canvas').or(
      page.locator('canvas').first()
    );
    await expect(mapCanvas).toBeVisible();
    
    // Check that map is interactive (has zoom controls)
    const zoomControls = page.locator('.mapboxgl-ctrl-zoom').or(
      page.locator('[data-testid="zoom-controls"]')
    );
    
    // Zoom controls might be custom or default Mapbox
    if (await zoomControls.isVisible()) {
      const zoomIn = zoomControls.locator('[data-testid="zoom-in"]').or(
        zoomControls.getByRole('button').first()
      );
      await expect(zoomIn).toBeVisible();
    }
  });

  test('displays directory entries as interactive map markers', async ({ page }) => {
    // Wait for markers to load
    await page.waitForTimeout(3000);
    
    // Look for map markers representing directory entries
    const markers = page.locator('.mapboxgl-marker').or(
      page.locator('[data-testid="map-marker"]')
    );
    ...
    const popup = page.locator('.mapboxgl-popup').or(
      page.locator('[data-testid="map-popup"]')
    );
  ...
  test('enables category filtering for spatial discovery', async ({ page }) => {
    ...
    const categoryFilters = page.locator('[data-testid="map-filters"]').or(
      page.locator('.map-filter-control')
    );
    
    if (await categoryFilters.isVisible()) {
      // Test restaurant filter
      const restaurantFilter = categoryFilters.getByRole('button', { name: /restaurant/i }).or(
        categoryFilters.getByText(/restaurant/i)
      );
      ...
      if (await restaurantFilter.isVisible()) {
        ...
        const visibleMarkers = page.locator('.mapboxgl-marker:visible').or(
          page.locator('[data-testid="map-marker"]:visible')
        );
  ...
  test('integrates with directory for seamless tourism flow', async ({ page }) => {
    ...
    if (await markers.count() > 0) {
      ...
      const viewDetailsLink = page.getByRole('link', { name: /view details|more info/i }).or(
        page.getByRole('button', { name: /view details|more info/i })
      );
      ...
      if (await viewDetailsLink.isVisible()) {
        ...
        const backToMapLink = page.getByRole('link', { name: /map/i }).or(
  ...
  test('performs well on mobile devices with touch controls', async ({ page, browserName }) => {
  ...
  test('handles poor connectivity gracefully', async ({ page }) => {
    ...
    const loadingIndicator = page.locator('[data-testid="map-loading"]').or(
      page.getByText(/loading|connecting/i)
    );
  ...
  test('integrates with towns page for regional exploration', async ({ page }) => {
    // Look for town markers or labels on the map
    const townMarkers = page.locator('[data-testid="town-marker"]').or(
      page.locator('.town-label, .city-label')
    );
    ...
    if (await townMarkers.count() > 0) {
      ...
      const townInfo = page.locator('[data-testid="town-popup"]').or(
        page.getByText(/Nova Sintra|Furna|Fajã de Água/i)
      );
...
test.describe('Map Performance for Tourism', () => {
  ...
  test('loads efficiently on various connection speeds', async ({ page }) => {
    ...
    const connectionProfiles = [
      { name: '3G', downloadThroughput: 1.5 * 1024 * 1024 / 8, uploadThroughput: 750 * 1024 / 8, latency: 150 },
    ...
    for (const profile of connectionProfiles) {
      ...
      await page.waitForSelector('[data-testid="interactive-map"]', { timeout: 30000 });
