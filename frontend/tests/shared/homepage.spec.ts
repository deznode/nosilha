...
test.describe('Homepage - Tourism Platform Entry Point', () => {
  ...
  test('displays compelling hero section for tourism discovery', async ({ page }) => {
    // Verify hero section captures Brava Island's essence
    const heroTitle = page.getByRole('heading', { name: /discover the soul of brava/i });
    ...
    const mapButton = page.getByRole('link', { name: /explore the interactive map/i });
  ...
  test('showcases featured highlights from directory', async ({ page }) => {
    ...
    const featuredSection = page.locator('section').filter({ hasText: 'Featured Highlights' });
  ...
  test('provides clear navigation to core tourism features', async ({ page }) => {
    // Verify Island Guide section with key platform features
    const islandGuide = page.locator('section').filter({ hasText: 'Your Comprehensive Guide to Brava' });
    await expect(islandGuide).toBeVisible();
    
    // Check for three core features
    const interactiveMapLink = page.getByRole('link', { name: /interactive map/i });
    const directoryLink = page.getByRole('link', { name: /complete directory/i });
    const historyLink = page.getByRole('link', { name: /rich history & culture/i });
  ...
  test('performs well for mobile tourists with limited bandwidth', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /discover the soul of brava/i })).toBeVisible();
  ...
  test('maintains accessibility for diverse tourism audience', async ({ page }) => {
  ...
  test('handles API failures gracefully with mock data fallback', async ({ page }) => {
    ...
    const hasCards = await page.locator('[data-testid="directory-card"]').count() > 0;
    const hasErrorMessage = await page.locator('text=temporarily unavailable').isVisible();
    
    expect(hasCards || hasErrorMessage).toBeTruthy();
  });

  test('supports Cape Verde cultural context and language', async ({ page }) => {
