...
test.describe("Directory Browsing - Core Tourism Discovery", () => {
  test("displays comprehensive directory of island businesses and landmarks", async ({
    page,
  }) => {
    ...
    const directoryGrid = page
      .locator('[data-testid="directory-grid"]')
      .or(
        page
          .locator(".grid")
          .filter({ hasText: /restaurant|hotel|beach|landmark/i })
      );
  ...
  test("enables category-specific browsing for targeted tourism discovery", async ({
    page,
  }) => {
    // Test restaurant category
    await page.goto("/directory/Restaurant");
    await expect(
      page.getByRole("heading", { name: /restaurants?/i })
    ).toBeVisible();
    ...
    for (const category of categories) {
      ...
      await expect(
        page.getByRole("heading", { name: new RegExp(category, "i") })
      ).toBeVisible();
  ...
  test("provides detailed information for tourism decision making", async ({
    page,
  }) => {
    await page.goto("/directory/all");
    ...
    if (pageContent?.toLowerCase().includes("restaurant")) {
      ...
      const hasHours =
        /\d{1,2}:\d{2}/.test(pageContent) || /hours?/i.test(pageContent);
  ...
  test("handles pagination and large directory efficiently", async ({
    page,
  }) => {
    await page.goto("/directory/all");
  ...
  test("optimized for mobile tourism browsing", async ({
    page,
    browserName,
  }) => {
    ...
    if (viewportSize && viewportSize.width <= 768) {
      ...
      const gridColumns = await page.evaluate(() => {
        const grid =
          document.querySelector('[data-testid="directory-grid"]') ||
    ...
    const categoryFilter = page
      .getByRole("link", { name: /restaurant/i })
      .or(page.getByRole("button", { name: /restaurant/i }));
  ...
  test("maintains functionality with intermittent connectivity", async ({
    ...
    if (await restaurantLink.isVisible()) {
      ...
      const hasOfflineMessage = await page
    ...
    const restoredCards = await page
      .locator('[data-testid="directory-card"]')
      .count();
    expect(restoredCards).toBeGreaterThanOrEqual(initialCards);
  });

  test("provides search and filtering for efficient discovery", async ({
    page,
  }) => {
    await page.goto("/directory/all");
  ...
  test("integrates with map view for spatial discovery", async ({ page }) => {
    ...
    const mapButton = page
      ...
      .or(page.getByRole("button", { name: /map/i }))
      .or(page.getByText(/view on map/i));
  ...
  test("displays accurate ratings and reviews for tourism trust", async ({
    page,
  }) => {
    await page.goto("/directory/all");
    ...
    if (await firstCard.isVisible()) {
      ...
      const ratingElement = firstCard
        .locator('[data-testid="rating"]')
        .or(
          firstCard
            .locator(".rating, .stars")
            .or(firstCard.locator("text=/[0-9]\.[0-9].*star/i"))
        );
...
test.describe("Directory - Mobile Tourism Experience", () => {
  ...
  test("enables easy category switching on mobile", async ({ page }) => {
    ...
    const categoryNav = page
      .locator('[data-testid="category-nav"]')
      .or(
        page
          .locator("nav")
          .filter({ hasText: /restaurant|hotel|beach|landmark/i })
    ...
    if (await categoryNav.isVisible()) {
      const restaurantTab = categoryNav.getByRole("link", {
        name: /restaurant/i,
      });
