...
export function setup() {
  ...
  const entriesResponse = http.get(
    `${API_BASE_URL}/api/v1/directory/entries?size=50`
  );

  if (entriesResponse.status === 200) {
    const data = JSON.parse(entriesResponse.body);
    const sampleSlugs = data.data
      ? data.data.slice(0, 10).map((entry) => entry.slug)
      : [];
    console.log(
      `📋 Retrieved ${sampleSlugs.length} sample entry slugs for testing`
    );
    return { sampleSlugs };
  }
...
function performTourismBrowsingSession(data, config, params) {
  ...
  if (
    data.sampleSlugs &&
    data.sampleSlugs.length > 0 &&
    config.browse_depth > 1
  ) {
...
function viewIndividualEntry(sampleSlugs, params) {
  ...
  const success = check(response, {
    ...
    "entry details complete": (r) => {
      ...
      const body = JSON.parse(r.body);
      return (
        body.success && body.data && body.data.name && body.data.description
      );
    },
