...
export function setup() {
  ...
  if (frontendHealth.status !== 200) {
    console.warn(
      `Frontend not accessible: ${frontendHealth.status} - API-only testing`
    );
  }
  ...
  try {
    const entriesResponse = http.get(
      `${API_BASE_URL}/api/v1/directory/entries?size=20`
    );
...
export function mobileQuickAccessJourney(data) {
  ...
  try {
    ...
    if (
      quickStep.success &&
      data.sampleData.entries &&
      data.sampleData.entries.length > 0
    ) {
...
export function planningResearchJourney(data) {
  ...
  try {
    ...
    const steps = [
      ...
      {
        name: "detailed_entries",
        fn: () =>
          data.sampleData.entries
            ? performMultipleEntryAnalysis(data.sampleData.entries)
            : { success: true },
      },
...
function performHomepageDiscovery() {
  ...
  if (__ENV.FRONTEND_AVAILABLE !== "false") {
    ...
    return {
      success: check(response, {
        ...
        "homepage has content": (r) =>
          r.body.includes("Discover") || r.body.includes("Brava"),
      }),
    ...
    const response = http.get(
      `${API_BASE_URL}/api/v1/directory/entries?size=4`,
      params
    );
...
function performEntryInvestigation(entries) {
  ...
  const response = http.get(
    `${API_BASE_URL}/api/v1/directory/slug/${entry.slug}`,
    params
  );
...
function performMapExploration() {
  ...
  const response = http.get(
    `${API_BASE_URL}/api/v1/directory/entries?size=100`,
    params
  );
...
function performTownsExploration() {
  ...
  const success = check(response, {
    ...
    "towns have cultural content": (r) => {
      const body = JSON.parse(r.body);
      return (
        body.success && body.data && body.data[0] && body.data[0].description
      );
    },
  });
...
function performCulturalInvestigation(towns) {
  ...
  const response = http.get(
    `${API_BASE_URL}/api/v1/towns/slug/${town.slug}`,
    params
  );
...
function performContextualDirectoryExploration() {
  ...
  const response = http.get(
    `${API_BASE_URL}/api/v1/directory/entries?size=10`,
    params
  );
...
function completeJourney(journeyType, success, errorMessage) {
  ...
  console.log(
    `✅ ${journeyType} journey completed: ${completionRate * 100}% success in ${journeyDuration}ms`
  );
...
export function teardown(data) {
  ...
  console.log(
    "   - Cultural Exploration: Towns → Cultural Content → Related Entries"
  );
