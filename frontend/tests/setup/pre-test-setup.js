...
async function performHealthChecks() {
  ...
  try {
    const apiHealthy = await checkService(
      CONFIG.API_URL + "/actuator/health",
      "Backend API"
    );
    ...
    if (apiHealthy) {
      ...
      for (const endpoint of criticalEndpoints) {
        const endpointHealthy = await checkService(
...
async function validateTestData() {
  ...
  try {
    ...
    if (mockDataExists) {
      ...
      console.log(
        "   ⚠️  Mock data directory not found - will be created during tests"
      );
    }
  } catch (error) {
