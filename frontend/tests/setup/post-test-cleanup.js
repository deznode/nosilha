...
async function main() {
  ...
  try {
    ...
    const summary = await generateSummaryReport(testResults, performanceMetrics);
...
async function collectTestResults() {
  ...
  try {
    ...
    const lhrFiles = lighthouseFiles.filter(file => file.startsWith('lhr-') && file.endsWith('.json'));
    
    if (lhrFiles.length > 0) {
      const lighthouseResults = [];
      
      for (const file of lhrFiles.slice(0, 5)) { // Limit to 5 most recent
      ...
      console.log(`   ✓ Lighthouse results collected (${lhrFiles.length} audits)`);
    }
  } catch (error) {
  ...
  try {
    ...
    if (Object.keys(k6Results).length > 0) {
      ...
      console.log(`   ✓ K6 results collected (${Object.keys(k6Results).length} tests)`);
    }
  } catch (error) {
...
async function generateSummaryReport(testResults, performanceMetrics) {
  ...
  const summary = {
    ...
    execution: {
      playwright: testResults.playwright ? {
        totalTests: testResults.playwright.suites?.reduce((acc, suite) => acc + suite.tests?.length || 0, 0) || 0,
      ...
      lighthouse: testResults.lighthouse ? {
        ...
        avgPerformanceScore: performanceMetrics.coreWebVitals ? 
          performanceMetrics.coreWebVitals.reduce((sum, item) => sum + item.performance, 0) / performanceMetrics.coreWebVitals.length : null
      } : null,
      
      k6: testResults.k6 ? {
        testsRun: Object.keys(testResults.k6).length,
        totalRequests: Object.values(testResults.k6).reduce((sum, test) => 
          sum + (test.metrics?.http_reqs?.count || 0), 0)
      } : null
    },
    ...
    performance: {
      coreWebVitals: performanceMetrics.coreWebVitals ? {
        avgLCP: performanceMetrics.coreWebVitals.reduce((sum, item) => sum + (item.lcp || 0), 0) / performanceMetrics.coreWebVitals.length,
        avgCLS: performanceMetrics.coreWebVitals.reduce((sum, item) => sum + (item.cls || 0), 0) / performanceMetrics.coreWebVitals.length,
        avgPerformanceScore: performanceMetrics.coreWebVitals.reduce((sum, item) => sum + item.performance, 0) / performanceMetrics.coreWebVitals.length
      } : null,
      
      apiPerformance: performanceMetrics.apiPerformance ? {
        avgResponseTime: Object.values(performanceMetrics.apiPerformance).reduce((sum, test) => 
          sum + (test.avgResponseTime || 0), 0) / Object.keys(performanceMetrics.apiPerformance).length,
        avgErrorRate: Object.values(performanceMetrics.apiPerformance).reduce((sum, test) => 
          sum + (test.errorRate || 0), 0) / Object.keys(performanceMetrics.apiPerformance).length
      } : null
    },
    ...
    tourismMetrics: {
      mobileOptimization: performanceMetrics.coreWebVitals ? 
        performanceMetrics.coreWebVitals.filter(item => item.performance > 75).length / performanceMetrics.coreWebVitals.length : null,
      
      culturalContentAccess: testResults.k6?.['towns-api-load'] ? 
        testResults.k6['towns-api-load'].metrics?.town_api_errors?.rate < 0.01 : null,
      
      directoryDiscovery: testResults.k6?.['directory-api-load'] ? 
        testResults.k6['directory-api-load'].metrics?.api_errors?.rate < 0.02 : null
    },
  ...
  if (summary.performance.coreWebVitals?.avgPerformanceScore < 75) {
    summary.recommendations.push('Improve Core Web Vitals for better mobile tourism experience');
  }
  
  if (summary.performance.apiPerformance?.avgResponseTime > 1000) {
    summary.recommendations.push('Optimize API response times for tourism platform responsiveness');
  }
  
  if (summary.tourismMetrics.mobileOptimization < 0.8) {
    summary.recommendations.push('Focus on mobile optimization for traveling tourists');
  }
  
  // Save summary report
  try {
    await fs.mkdir(CONFIG.OUTPUT_DIR, { recursive: true });
    const summaryPath = path.join(CONFIG.OUTPUT_DIR, 'integration-test-summary.json');
...
async function prepareCIIntegration(summary) {
  ...
  const ciData = {
    ...
    metrics: {
      ...
      performanceScore: summary.performance.coreWebVitals?.avgPerformanceScore || 0,
    ...
    tourismQuality: {
      ...
      culturalContentReliability: summary.tourismMetrics.culturalContentAccess || false,
      directoryFunctionality: summary.tourismMetrics.directoryDiscovery || false
    },
  ...
  ciData.success = ciData.metrics.testsFailed === 0 && 
                  ciData.metrics.performanceScore > 75 &&
                  ciData.metrics.errorRate < 0.02;
  
  ciData.deploymentReady = ciData.success && 
...
async function organizeArtifacts() {
  ...
  try {
    ...
    const playwrightReportExists = await fs.access(path.join(CONFIG.REPORTS_DIR, 'index.html')).then(() => true).catch(() => false);
    if (playwrightReportExists) {
      ...
      await fs.writeFile('test-output/reports/playwright-report-info.json', JSON.stringify(reportInfo, null, 2));
    }
    
    // Copy performance metrics
    const performanceFiles = await fs.readdir(CONFIG.K6_RESULTS_DIR).catch(() => []);
...
function displayResults(summary) {
  ...
  if (summary.execution.playwright) {
    ...
    console.log(`   Duration: ${Math.round(summary.execution.playwright.duration / 1000)}s`);
  }
  
  if (summary.execution.lighthouse) {
    console.log(`\n🔍 Lighthouse Audits:`);
    console.log(`   Audits Run: ${summary.execution.lighthouse.auditsRun}`);
    console.log(`   Avg Performance Score: ${Math.round(summary.execution.lighthouse.avgPerformanceScore || 0)}/100`);
  }
  ...
  if (summary.performance.coreWebVitals) {
    console.log(`   Average LCP: ${Math.round(summary.performance.coreWebVitals.avgLCP)}ms`);
    console.log(`   Average CLS: ${summary.performance.coreWebVitals.avgCLS.toFixed(3)}`);
    console.log(`   Performance Score: ${Math.round(summary.performance.coreWebVitals.avgPerformanceScore)}/100`);
  }
  
  if (summary.performance.apiPerformance) {
    console.log(`   API Response Time: ${Math.round(summary.performance.apiPerformance.avgResponseTime)}ms`);
    console.log(`   API Error Rate: ${(summary.performance.apiPerformance.avgErrorRate * 100).toFixed(2)}%`);
  }
  ...
  if (summary.tourismMetrics.mobileOptimization !== null) {
    console.log(`   Mobile Optimization: ${Math.round(summary.tourismMetrics.mobileOptimization * 100)}%`);
  }
  
  console.log(`   Cultural Content Access: ${summary.tourismMetrics.culturalContentAccess ? '✅' : '❌'}`);
  console.log(`   Directory Discovery: ${summary.tourismMetrics.directoryDiscovery ? '✅' : '❌'}`);
