...
async function globalSetup(config: FullConfig) {
  ...
  try {
    ...
    try {
      ...
      throw new Error('Frontend application must be running before tests. Run: npm run dev');
    }
    ...
    try {
      const healthResponse = await page.request.get(`${apiUrl}/actuator/health`);
      if (healthResponse.ok()) {
        console.log('✅ Backend API is healthy');
      } else {
        console.warn('⚠️ Backend API health check failed - falling back to mock data');
      }
    } catch (error) {
      console.warn('⚠️ Backend API not accessible - tests will use mock data fallback');
    }
    ...
    try {
      ...
      if (!mapboxToken || mapboxToken.startsWith('your_mapbox_token')) {
        console.warn('⚠️ Mapbox token not configured - map tests will be limited');
      } else {
