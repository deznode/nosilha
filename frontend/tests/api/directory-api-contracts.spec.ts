...
test.describe('Directory API Contracts', () => {
  ...
  test('GET /api/v1/directory/entries returns paginated directory entries', async ({ request }) => {
    const response = await request.get(`${apiBaseUrl}/api/v1/directory/entries`);
    ...
    if (data.data.length > 0) {
      ...
      expect(['Restaurant', 'Hotel', 'Beach', 'Landmark']).toContain(firstEntry.category);
  ...
  test('GET /api/v1/directory/entries supports category filtering', async ({ request }) => {
    const categories = ['Restaurant', 'Hotel', 'Beach', 'Landmark'];
    
    for (const category of categories) {
      const response = await request.get(`${apiBaseUrl}/api/v1/directory/entries?category=${category}`);
  ...
  test('GET /api/v1/directory/entries supports pagination parameters', async ({ request }) => {
    // Test with different page sizes
    const pageSizes = [5, 10, 20];
    
    for (const size of pageSizes) {
      const response = await request.get(`${apiBaseUrl}/api/v1/directory/entries?page=0&size=${size}`);
    ...
    const firstPageResponse = await request.get(`${apiBaseUrl}/api/v1/directory/entries?page=0&size=5`);
    const firstPageData: PagedApiResponse<DirectoryEntryDto> = await firstPageResponse.json();
    
    if (firstPageData.pagination.totalPages > 1) {
      const secondPageResponse = await request.get(`${apiBaseUrl}/api/v1/directory/entries?page=1&size=5`);
      const secondPageData: PagedApiResponse<DirectoryEntryDto> = await secondPageResponse.json();
  ...
  test('GET /api/v1/directory/slug/{slug} returns specific entry', async ({ request }) => {
    // First, get a list of entries to find a valid slug
    const entriesResponse = await request.get(`${apiBaseUrl}/api/v1/directory/entries?size=5`);
    const entriesData: PagedApiResponse<DirectoryEntryDto> = await entriesResponse.json();
    
    if (entriesData.data.length > 0) {
      const testEntry = entriesData.data[0];
      
      // Test getting entry by slug
      const response = await request.get(`${apiBaseUrl}/api/v1/directory/slug/${testEntry.slug}`);
  ...
  test('GET /api/v1/directory/slug/{slug} returns 404 for non-existent entry', async ({ request }) => {
    const response = await request.get(`${apiBaseUrl}/api/v1/directory/slug/non-existent-entry-slug`);
  ...
  test('POST /api/v1/directory/entries validates required fields', async ({ request }) => {
    ...
    const response = await request.post(`${apiBaseUrl}/api/v1/directory/entries`, {
      data: invalidEntry,
      headers: {
  ...
  test('POST /api/v1/directory/entries validates geographic constraints', async ({ request }) => {
  ...
  test('API responses include proper CORS headers', async ({ request }) => {
    ...
    expect(['*', 'http://localhost:3000', process.env.FRONTEND_URL].some(origin => 
  ...
  test('API performance meets tourism platform requirements', async ({ request }) => {
  ...
  test('API maintains data consistency across operations', async ({ request }) => {
    // Get all entries
    const allEntriesResponse = await request.get(`${apiBaseUrl}/api/v1/directory/entries`);
    const allEntries: PagedApiResponse<DirectoryEntryDto> = await allEntriesResponse.json();
    
    if (allEntries.data.length > 0) {
      const testEntry = allEntries.data[0];
      
      // Get same entry by slug
      const slugResponse = await request.get(`${apiBaseUrl}/api/v1/directory/slug/${testEntry.slug}`);
      const slugEntry: ApiResponse<DirectoryEntryDto> = await slugResponse.json();
  ...
  test('API supports case-insensitive category filtering', async ({ request }) => {
    ...
    for (const categoryVariation of categoryVariations) {
      const response = await request.get(`${apiBaseUrl}/api/v1/directory/entries?category=${categoryVariation}`);
