...
test.describe('Towns API Contracts', () => {
  ...
  test('GET /api/v1/towns/all returns all Brava Island towns', async ({ request }) => {
  ...
  test('GET /api/v1/towns/all includes key Brava Island communities', async ({ request }) => {
    const response = await request.get(`${apiBaseUrl}/api/v1/towns/all`);
    const data: ApiResponse<TownDto[]> = await response.json();
    ...
    const hasCapital = townNames.some(name => name.includes('nova sintra')) ||
  ...
  test('GET /api/v1/towns/slug/{slug} returns specific town details', async ({ request }) => {
    // First get all towns to find valid slugs
    const allTownsResponse = await request.get(`${apiBaseUrl}/api/v1/towns/all`);
    ...
    if (allTowns.data.length > 0) {
      ...
      const response = await request.get(`${apiBaseUrl}/api/v1/towns/slug/${testTown.slug}`);
  ...
  test('GET /api/v1/towns/slug/nova-sintra returns capital town details', async ({ request }) => {
    const response = await request.get(`${apiBaseUrl}/api/v1/towns/slug/nova-sintra`);
    ...
    if (response.ok()) {
      ...
      const hasCapitalMentions = description.includes('capital') || 
      ...
      console.log('Nova Sintra endpoint not available - towns may use different slugs');
    }
  });

  test('GET /api/v1/towns/slug/{slug} returns 404 for non-existent town', async ({ request }) => {
    const response = await request.get(`${apiBaseUrl}/api/v1/towns/slug/non-existent-town`);
  ...
  test('Towns API provides culturally authentic information', async ({ request }) => {
    const response = await request.get(`${apiBaseUrl}/api/v1/towns/all`);
    const data: ApiResponse<TownDto[]> = await response.json();
    ...
    const allText = data.data.map(town => 
      `${town.description} ${town.highlights.join(' ')}`
    ).join(' ').toLowerCase();
    ...
    const hasCulturalContent = culturalTerms.some(term => allText.includes(term));
    ...
    const hasInappropriateContent = inappropriateTerms.some(term => allText.includes(term));
    expect(hasInappropriateContent).toBe(false);
  });

  test('Towns API includes practical tourism information', async ({ request }) => {
    const response = await request.get(`${apiBaseUrl}/api/v1/towns/all`);
    const data: ApiResponse<TownDto[]> = await response.json();
  ...
  test('Towns API performance meets tourism requirements', async ({ request }) => {
  ...
  test('Towns API supports slugs with special characters', async ({ request }) => {
    const response = await request.get(`${apiBaseUrl}/api/v1/towns/all`);
    const data: ApiResponse<TownDto[]> = await response.json();
    
    // Look for towns with special characters (like Fajã de Água)
    const townWithSpecialChars = data.data.find(town => 
    ...
    if (townWithSpecialChars) {
      ...
      const slugResponse = await request.get(`${apiBaseUrl}/api/v1/towns/slug/${townWithSpecialChars.slug}`);
  ...
  test('Towns API handles edge cases gracefully', async ({ request }) => {
    ...
    for (const invalidSlug of invalidSlugs) {
      const response = await request.get(`${apiBaseUrl}/api/v1/towns/slug/${encodeURIComponent(invalidSlug)}`);
  ...
  test('Towns API maintains referential integrity with directory', async ({ request }) => {
    // Get all towns
    ...
    const directoryResponse = await request.get(`${apiBaseUrl}/api/v1/directory/entries`);
    ...
    if (directoryResponse.ok()) {
      ...
      if (directoryData.data && directoryData.data.length > 0) {
        ...
        directoryData.data.forEach((entry: any) => {
          if (entry.town) {
            const townExists = townNames.some(townName => 
              townName.toLowerCase() === entry.town.toLowerCase()
            );
            
            // Directory entries should reference valid towns
            // (This might not be enforced, but it's good data integrity)
            if (!townExists) {
              console.warn(`Directory entry references unknown town: ${entry.town}`);
            }
          }
        });
      }
...
test.describe('Towns API Error Handling', () => {
  ...
  test('API provides proper error responses for malformed requests', async ({ request }) => {
  ...
  test('API handles server errors gracefully', async ({ request }) => {
    ...
    const response = await request.get(`${apiBaseUrl}/api/v1/towns/slug/${'x'.repeat(1000)}`);
