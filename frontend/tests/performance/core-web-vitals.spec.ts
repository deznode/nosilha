...
test.describe('Core Web Vitals - Tourism Performance', () => {
      ...
      return new Promise<WebVitalsMetrics>((resolve) => {
        ...
        const checkComplete = () => {
          ...
          if (metricsCollected >= totalMetrics || 
              (metrics.lcp && metrics.fid !== undefined && metrics.cls !== undefined)) {
            resolve({
              lcp: metrics.lcp || 0,
              fid: metrics.fid || 0,
              cls: metrics.cls || 0,
        ...
        if (typeof (window as any).webVitals !== 'undefined') {
          const { getLCP, getFID, getCLS, getFCP, getTTFB } = (window as any).webVitals;
  ...
  test('homepage meets Core Web Vitals thresholds for tourism UX', async ({ page }) => {
  ...
  test('directory browsing maintains performance under load', async ({ page }) => {
  ...
  test('interactive map performance for mobile tourists', async ({ page }) => {
    ...
    const mapContainer = page.locator('.mapboxgl-canvas').or(
  ...
  test('performance across different connection speeds', async ({ page }) => {
    ...
    for (const profile of connectionProfiles) {
      ...
      const lcpThreshold = profile.name === 'Slow 3G' ? 4000 : 
                          profile.name === '3G' ? 3000 : 2500;
  ...
  test('image optimization impact on performance', async ({ page }) => {
    ...
    for (const img of images) {
      ...
      if (src) {
        ...
        const isNextJsOptimized = src.includes('/_next/image') || src.includes('/_next/static');
  ...
  test('font loading impact on Core Web Vitals', async ({ page }) => {
    ...
    const fontLoadingStrategy = await page.evaluate(() => {
      const linkTags = Array.from(document.querySelectorAll('link[rel="preload"][as="font"]'));
      const fontDisplay = Array.from(document.styleSheets).some(sheet => {
        try {
          const rules = sheet.cssRules || sheet.rules;
          return Array.from(rules).some(rule => 
            rule.cssText && rule.cssText.includes('font-display')
          );
        } catch (e) {
          return false;
        }
...
test.describe('Mobile Core Web Vitals - Tourism Focus', () => {
  ...
  test('mobile homepage performance for traveling tourists', async ({ page }) => {
