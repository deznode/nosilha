...
test.describe("Content Action Toolbar - Keyboard Accessibility", () => {
  ...
  test("T049: Arrow key navigation between action buttons", async ({
    page,
  }) => {
    // Find the toolbar
    const toolbar = page.locator(
      '[role="toolbar"][aria-label="Content actions"]'
    );
  ...
  test("T049: Home and End keys jump to first and last buttons", async ({
    page,
  }) => {
    const toolbar = page.locator(
      '[role="toolbar"][aria-label="Content actions"]'
  ...
  test("T050: Focus indicators are visible and meet contrast requirements", async ({
    page,
  }) => {
    const toolbar = page.locator(
      '[role="toolbar"][aria-label="Content actions"]'
  ...
  test("T053: Keyboard navigation works for reaction buttons", async ({
    page,
  }) => {
    const toolbar = page.locator(
      '[role="toolbar"][aria-label="Content actions"]'
    ...
    const reactionGroup = toolbar.locator(
    ...
    for (let i = 0; i < 20; i++) {
      const isFocused = await reactionButtons
        .first()
...
test.describe("Content Action Toolbar - Screen Reader Compatibility", () => {
  ...
  test("T055: Action buttons have descriptive ARIA labels", async ({
    page,
  }) => {
    const toolbar = page.locator(
      '[role="toolbar"][aria-label="Content actions"]'
  ...
  test("T055: Reaction buttons have comprehensive ARIA labels with counts", async ({
    page,
  }) => {
    const toolbar = page.locator(
      '[role="toolbar"][aria-label="Content actions"]'
  ...
  test("T055: ARIA live regions announce interaction feedback", async ({
    page,
  }) => {
  ...
  test("T055: Emojis have aria-hidden to prevent screen reader announcement", async ({
    page,
  }) => {
    const toolbar = page.locator(
      '[role="toolbar"][aria-label="Content actions"]'
...
test.describe("Content Action Toolbar - Keyboard-Only User Flow", () => {
  test("T049-T053: Complete keyboard-only interaction flow", async ({
    page,
  }) => {
    await page.goto("/directory/entry/djababas-eco-lodge");
    await page.waitForLoadState("networkidle");
