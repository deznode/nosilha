...
test.describe('Directory Browsing - Main Listing', () => {
  test('should load directory listing page with entries', async ({ page }) => {
    ...
    const directoryCards = page.locator('[class*="DirectoryCard"], article, [data-testid="directory-entry"]');
  ...
  test('should display server-rendered content on initial load', async ({ page }) => {
  ...
  test('should display entry cards with required information', async ({ page }) => {
    ...
    if (cardCount > 0) {
      ...
      // 1. Name/Title
      const title = firstCard.locator('h2, h3, [class*="title"], [class*="name"]').first();
...
test.describe('Directory Browsing - Entry Detail Pages', () => {
  ...
  test('should display complete entry information on detail page', async ({ page }) => {
    ...
    if (firstLinkExists) {
      ...
      if (href) {
        ...
        const category = page.locator('[class*="badge"], [class*="tag"], [class*="category"]').first();
  ...
  test('should be responsive on mobile viewport - detail page', async ({ page }) => {
    await page.setViewportSize({ width: 390, height: 844 });
...
test.describe('Directory Browsing - Navigation', () => {
  test('should have working breadcrumb navigation', async ({ page }) => {
    ...
    const breadcrumbs = page.locator('[aria-label*="breadcrumb"], nav[class*="breadcrumb"]');
...
test.describe('Directory Browsing - Performance', () => {
  ...
  test('should use Next.js ISR caching for better performance', async ({ page }) => {
    // First visit
