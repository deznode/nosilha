...
test.describe("Admin Content Creation - Access Control", () => {
  ...
  test("should allow authenticated admin users to access admin area", async ({
    page,
  }) => {
    // Log in as admin
  ...
  test("should display create new entry button in admin area", async ({
    page,
  }) => {
    await loginAsAdmin(page);
    await page.goto("/admin");
...
test.describe("Admin Content Creation - Form Display", () => {
  test("should display content creation form", async ({ page }) => {
    ...
    if (!formFound) {
      ...
      const createButton = page
        .locator('button:has-text("Create"), button:has-text("Add New")')
        .first();
  ...
  test("should have all required form fields for directory entry", async ({
    page,
  }) => {
    await loginAsAdmin(page);
    ...
    for (const path of createPaths) {
      ...
      if (hasNameField) {
        ...
        const categoryField = page
          ...
          .first();
        const hasCategoryField = await categoryField
          .isVisible()
          .catch(() => false);
        ...
        const descriptionField = page
          ...
          .first();
        const hasDescriptionField = await descriptionField
          .isVisible()
          .catch(() => false);

        // Form should have at least name and one other field
        expect(
          hasNameField && (hasCategoryField || hasDescriptionField)
        ).toBeTruthy();
        break;
      }
    }
...
test.describe("Admin Content Creation - Form Submission", () => {
    ...
    if (hasNameField) {
      ...
      if (await descriptionField.isVisible().catch(() => false)) {
        await descriptionField.fill(
          "This is a test entry created by E2E tests."
        );
      }
  ...
  test("should show loading state during form submission", async ({ page }) => {
    ...
    if (await nameField.isVisible().catch(() => false)) {
      ...
      expect(
        isDisabled || hasLoadingText?.toLowerCase().includes("saving")
      ).toBeTruthy();
    }
  });
