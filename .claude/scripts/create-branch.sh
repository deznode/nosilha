#!/usr/bin/env bash
# Branch creation script for Nos Ilha project
...
get_next_feature_number() {
    ...
    local specs_dir="$monorepo_root/plan/specs"
...
create_branch() {
    ...
    if git checkout -b "$branch_name"; then
        ...
        local feature_dir="$monorepo_root/plan/specs/$branch_name"
        mkdir -p "$feature_dir"
...
main() {
    ...
    echo "📁 Spec directory: plan/specs/$branch_name"
    echo ""
    echo "Next steps:"
    ...
    if [[ "$JSON_MODE" == "true" ]]; then
        json_output "success" "Branch created" "branch=$branch_name" "feature_num=$feature_num" "spec_dir=plan/specs/$branch_name"
    fi
}
