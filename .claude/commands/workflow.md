...
- **spec-name**: Name of a spec kit directory in `plan/specs/{spec-name}/`
...
2. **Spec Kit**: A spec name referencing `plan/specs/{spec-name}/` (single word or hyphenated)

### Loading Spec Kit Files

If the input is a spec name (e.g., "oauth-integration", "payment-gateway"), load both files:

!`cat plan/specs/$SPEC_NAME/spec.md 2>/dev/null || echo "Spec not found: plan/specs/$SPEC_NAME/spec.md"`
!`cat plan/specs/$SPEC_NAME/tasks.md 2>/dev/null || echo "Tasks not found: plan/specs/$SPEC_NAME/tasks.md"`
...
Spec Directory: [plan/specs/{name}/ if using spec kit]
...
**Spec Directory**: [plan/specs/{name}/ if applicable]
**Enhancement Flags**: [Active flags]
**Thinking Mode**: [Based on flags]

## Understanding Your Request

[If Spec Kit]: I've analyzed spec.md and tasks.md from plan/specs/{name}/ and understand:
...
When working with a spec kit from plan/specs/{spec-name}/:
