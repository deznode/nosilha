...
class SuggestionControllerTest {
    ...
    fun `submitSuggestion with valid data should create suggestion and return 201`() {
        ...
        mockMvc.perform(
            ...
            .andExpect(jsonPath("$.message").value(org.hamcrest.Matchers.containsString("Thank you")))

        // Verify suggestion was persisted to database
        val suggestions = suggestionRepository.findAll()
        assert(suggestions.size == 1)
        assert(suggestions[0].name == "Maria Santos")
        assert(suggestions[0].email == "maria.santos@example.com")
        assert(suggestions[0].suggestionType == SuggestionType.CORRECTION)
    }
