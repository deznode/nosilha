...
export function FeaturedStoryCard({ item }: FeaturedStoryCardProps) {
  return (
    ...
    >
      {/* Image Background */}
      <div className="absolute inset-0 bg-mist-200">
        <Image
          src={item.image}
          alt={item.title}
          fill
      ...
      <div className="absolute bottom-0 left-0 w-full translate-y-2 transform p-6 text-white transition-transform duration-300 group-hover:translate-y-0 md:p-8">
        ...
        <p className="mb-4 line-clamp-2 text-sm text-mist-200 opacity-0 transition-opacity delay-100 duration-500 group-hover:opacity-100">
          {item.description}
        </p>
